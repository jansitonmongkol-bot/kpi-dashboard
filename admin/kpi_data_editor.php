    ...
    if (empty($sql) || strpos($up, 'SELECT') !== 0) {
        ...
        $stmt = $conn->prepare("
            ...
            ON DUPLICATE KEY UPDATE
                ...
                trend_sql=VALUES(trend_sql), trend_db=VALUES(trend_db),
        ...
        ");
        // 25 params: i s s s i  s s  s s s s  s s  s s s s s  s s s s s  i i
        $trendSql = trim($_POST['trend_sql'] ?? '');
        $trendDb  = ($_POST['trend_db'] ?? '') === 'hdc' ? 'hdc' : 'kpi';
        $stmt->bind_param('isssissssssssssssssssssii',
...
<div class="tab-pane fade" id="tabChart">
    ...
    $savedTrendSql = $qc['trend_sql']      ?? '';
    $savedTrendDb  = $qc['trend_db']       ?? 'kpi';
    ?>
    ...
    <form method="post" id="formChart">
        ...
        <input type="hidden" name="trend_sql"      id="hidTrendSql"    value="<?= htmlspecialchars($savedTrendSql) ?>">
        <input type="hidden" name="trend_db"       id="hidTrendDb"     value="<?= htmlspecialchars($savedTrendDb)  ?>">
        ...
        <div class="chart-config-card mb-4">
            <div class="chart-config-header" style="--accent:#7c3aed">
                ...
                <span class="ms-auto d-flex align-items-center gap-2">
                    <!-- toggle: ใช้ SQL หลัก หรือ SQL แยก -->
                    <div class="form-check form-switch mb-0 me-1">
                        <input class="form-check-input" type="checkbox" id="trendSqlToggle"
                               <?= !empty($savedTrendSql) ? 'checked' : '' ?>
                               onchange="toggleTrendSqlMode(this.checked)">
                        <label class="form-check-label small text-white-50" for="trendSqlToggle">
                            SQL แยก
                        </label>
                    </div>
                </span>
            ...
            <div class="chart-config-body">
                ...
                <!-- ── โหมด SQL แยก ── -->
                <div id="trendSqlSection" style="display:<?= !empty($savedTrendSql)?'':'none' ?>">
                    <div class="small text-muted mb-2">
                        <i class="bi bi-info-circle me-1"></i>
                        เขียน SQL สำหรับ Trend Chart โดยเฉพาะ — ใช้ <code>:kpi_id</code> เป็น placeholder
                        · ต้องคืนอย่างน้อย 2 คอลัมน์ (label, value)
                    </div>
                    <div class="sql-topbar">
                        <div class="dots"><span class="dot-r"></span><span class="dot-y"></span><span class="dot-g"></span></div>
                        <span class="text-white-50 small me-auto">Trend SQL</span>
                        <select id="trendDbSel" class="form-select form-select-sm"
                                style="width:auto;background:#334155;color:#f8f8f2;border:1px solid #475569;font-size:.76rem"
                                onchange="document.getElementById('hidTrendDb').value=this.value">
                            <option value="kpi" <?= ($savedTrendDb==='kpi')?'selected':'' ?>>DB: kpi25</option>
                            <option value="hdc" <?= ($savedTrendDb==='hdc')?'selected':'' ?>>DB: hdc</option>
                        </select>
                        <button type="button" id="btnTestTrendSql"
                                class="btn btn-sm px-3"
                                style="background:#7c3aed;color:#fff;border:none;border-radius:6px;font-size:.78rem">
                            <span id="trendSqlSpinner" class="spinner-border spinner-border-sm d-none me-1"></span>
                            <i class="bi bi-play-fill"></i> ทดสอบ
                        </button>
                    </div>
                    <textarea id="trendSqlEd"><?= htmlspecialchars($savedTrendSql) ?></textarea>
                    <div id="trendSqlResult"  class="result-box" style="display:none"><div id="trendSqlResultInner"></div></div>
                    <div id="trendSqlStatus" class="result-status" style="display:none"><span id="trendSqlStatusText"></span></div>
                </div>

                <!-- ── Pickers (ใช้ได้ทั้ง 2 โหมด) ── -->
                <div class="row g-3 mt-1">
                    <div class="col-md-5">
                        <div class="small fw-semibold mb-2">
                            <i class="bi bi-arrows-horizontal me-1 text-purple"></i>แกน X — Label
                            <span class="text-muted fw-normal">(radio เลือก 1)</span>
                        </div>
                        <div id="trendXPicker" class="d-flex flex-wrap gap-2">
                            <?php if ($savedTrendX): ?>
                            <span class="chart-col-option cco-active">
                            ...
                            <?php else: ?>
                            <span class="text-muted small">กด "ทดสอบ" หรือ "โหลดคอลัมน์จาก SQL"</span>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="col-md-7">
                        <div class="small fw-semibold mb-2">
                            <i class="bi bi-arrows-vertical me-1 text-purple"></i>แกน Y — ค่าข้อมูล
                            <span class="text-muted fw-normal">(checkbox หลายตัว)</span>
                        </div>
                        <div id="trendYPicker" class="d-flex flex-wrap gap-2">
                            <?php if (!empty($savedTrendY)): ?>
                            <?php foreach ($savedTrendY as $col): ?>
                            <span class="chart-col-option cco-active"><span class="col-name"><?= htmlspecialchars($col) ?></span></span>
                            <?php endforeach; ?>
                            <span class="text-muted small align-self-center">กด "ทดสอบ" เพื่อเปลี่ยน</span>
                            <?php else: ?>
                            <span class="text-muted small">กด "ทดสอบ" หรือ "โหลดคอลัมน์จาก SQL"</span>
...
const initTrendSql = <?= json_encode($queryConfig['trend_sql'] ?? '') ?>;
const initTrendDb  = <?= json_encode($queryConfig['trend_db']  ?? 'kpi') ?>;
...
// ── Trend SQL editor ──────────────────────────────────────────
let trendSqlCm = null;
const trendSqlTa = document.getElementById('trendSqlEd');
if (trendSqlTa) {
    trendSqlCm = CodeMirror.fromTextArea(trendSqlTa, {
        mode: 'text/x-mysql', theme: 'dracula',
        lineNumbers: true, lineWrapping: true,
        extraKeys: { 'Ctrl-Enter': () => testTrendSql(), 'Cmd-Enter': () => testTrendSql() }
    });
    trendSqlCm.setSize(null, 160);
}

function toggleTrendSqlMode(on) {
    document.getElementById('trendSqlSection').style.display = on ? '' : 'none';
    if (!on) {
        // ล้าง SQL เมื่อ toggle off
        document.getElementById('hidTrendSql').value = '';
        if (trendSqlCm) trendSqlCm.setValue('');
    }
}

async function testTrendSql() {
    if (!trendSqlCm || !KPI_ID) return;
    const sql = trendSqlCm.getValue().trim();
    if (!sql) return;
    const db  = document.getElementById('trendDbSel')?.value || 'kpi';
    const sp  = document.getElementById('trendSqlSpinner');
    const resBox = document.getElementById('trendSqlResult');
    const resInner = document.getElementById('trendSqlResultInner');
    const stBox  = document.getElementById('trendSqlStatus');
    const stTxt  = document.getElementById('trendSqlStatusText');
    ...
    try {
        ...
        try { d = JSON.parse(text); }
            ...
            stBox.style.display = '';
            stTxt.innerHTML = '<span style="color:#f87171">Server error — response ไม่ใช่ JSON</span>';
            return;
        }
        sp?.classList.add('d-none');
        ...
        // แสดงตาราง preview
        let h = '<table><thead><tr>' + d.columns.map(col => `<th>${esc(col)}</th>`).join('') + '</tr></thead><tbody>';
        d.rows.slice(0, 10).forEach(row => {
            h += '<tr>' + row.map(v => `<td>${esc(String(v ?? ''))}</td>`).join('') + '</tr>';
        });
        h += '</tbody></table>';
        resInner.innerHTML = h;
        ...
        // sync hidden inputs
        document.getElementById('hidTrendSql').value = sql;
        document.getElementById('hidTrendDb').value  = db;

        // อัปเดต X/Y pickers ด้วยคอลัมน์จาก Trend SQL นี้
        renderPickerRadio('trendXPicker', 'hidTrendXCol', d.columns, 'tx', initTrendX);
        renderPickerCheck('trendYPicker', 'hidTrendYCols', d.columns, 'ty', initTrendY);
...
document.getElementById('btnTestTrendSql')?.addEventListener('click', testTrendSql);

// ── Sync trend_sql/trend_db ก่อน submit formChart ─────────────
document.getElementById('formChart')?.addEventListener('submit', () => {
    if (trendSqlCm && document.getElementById('trendSqlToggle')?.checked) {
        document.getElementById('hidTrendSql').value = trendSqlCm.getValue();
        document.getElementById('hidTrendDb').value  = document.getElementById('trendDbSel')?.value || 'kpi';
    } else {
        document.getElementById('hidTrendSql').value = '';
    }
});
